  ...
  <Teleport to="body">
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-90"
      ...
      <div v-if="open" class="fixed inset-0 flex items-center justify-center bg-black/70 z-1000 backdrop-blur-[6px]">
        <div class="relative border border-[#dc5b41] w-full max-w-md rounded-2xl shadow-2xl p-12 bg-black/80">
          ...
          <div class="flex justify-center gap-4">
            <button class="cursor-pointer text-[#fff] bg-[#dc5b41] px-8 py-2" @click="handleNextStep(true)">
              {{ t('button.yes') }}
            </button>
            <button class="cursor-pointer text-[#fff] bg-[#dc5b41] px-8 py-2" @click="handleNextStep(false)">
              {{ t('button.no') }}
            </button>
          </div>
        </div>
