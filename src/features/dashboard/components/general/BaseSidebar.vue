<template>
  ...
  >
    ...
    >
      <button
        class="cursor-pointer p-1.5 hover:bg-white/10 rounded transition-all duration-200"
        @click="changeClose"
    ...
    <div class="h-full flex flex-col p-6 overflow-hidden">
      ...
      <div class="mb-8 transition-all duration-300">
        ...
        <div
          v-if="!isClose"
          class="h-px bg-gradient-to-r from-transparent via-[#dc5b41]/30 to-transparent mt-4"
        ></div>
        <div
          v-else
          class="h-px bg-gradient-to-r from-transparent via-[#dc5b41]/30 to-transparent mt-2 mx-2"
        ></div>
      </div>
        ...
        <ul class="space-y-2">
          <li v-for="data in navs" :key="data.name">
            <button
              ...
              @click="$emit('handleNav', data)"
              :title="isClose ? t(data.name) : ''"
            >
              <!-- Active Indicator -->
              <div
                v-if="isActive(data)"
                class="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-[#dc5b41] to-[#e66a4f] rounded-r"
              ></div>
