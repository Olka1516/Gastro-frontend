<template>
  <div class="flex h-full min-h-0 flex-col overflow-hidden p-4 sm:p-5 lg:p-6">
    <div v-if="!hideProfile" class="mb-6 transition-all duration-300 lg:mb-8">
      <div
        ...
        <div v-if="showExpandedContent" class="min-w-0 overflow-hidden">
          <h2 class="truncate text-base font-bold text-white sm:text-xl">{{ userInfo.placeName }}</h2>
      ...
      <ul class="space-y-1.5 sm:space-y-2">
        <li v-for="data in navs" :key="data.name">
          <button
            ...
            :title="isDesktop && isClose ? t(data.name) : ''"
            @click="$emit('nav', data)">
            <div
              v-if="isActive(data)"
    ...
    <div :class="['flex flex-col', isDesktop && isClose ? 'gap-2' : '']">
      <button
        type="button"
        :class="[
        ...
        :title="isDesktop && isClose ? t('button.logOut') : undefined"
