  ...
  <div
    ...
    <div v-if="dishes.length === 0"
      ...
      <div class="flex flex-col items-center gap-4 sm:gap-6">
        <div
          class="flex h-24 w-24 items-center justify-center rounded-lg bg-gradient-to-br from-[#dc5b41] to-[#e66a4f] sm:h-32 sm:w-32">
    ...
    <div v-else class="grid grid-cols-1 gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-3">
      <div v-for="data in dishes.slice((paginationPage - 1) * size, paginationPage * size)" :key="data.id"
        class="group flex h-full flex-col overflow-hidden rounded-lg border border-[#2a2930] bg-gradient-to-br from-[#1a191f] to-[#0f0f11] transition-all duration-300 hover:scale-[1.02] hover:border-[#dc5b41]/50">
        ...
        <div class="flex flex-1 flex-col gap-3 p-4 sm:gap-4 sm:p-6">
          ...
          <div class="mt-auto flex items-center gap-2 border-t border-[#2a2930] pt-3 sm:pt-4">
            ...
            <button type="button"
              ...
              :aria-label="t('dashboard.tableHead.deleteMeal')" @click="changeDeleteValue(data.id)">
    ...
    <ManageDishPremium v-model:dish="editDish" v-model:openManage="openManage" text="dashboard.editText" :error
      @handleProcess="(value) => editMeal(value)" />
    <ManageDishPremium v-model:dish="newDish" v-model:openManage="openAdd" text="dashboard.addText" :error
...
const deleteMeal = async (value: boolean) => {
  if (value && mealId.value) {
    const success = await premiumDashboardStore.deleteDish(mealId.value)
    if (success) {
      await getDishes()
    } else {
      console.error('Failed to delete meal:', premiumDashboardStore.error)
    }
  }
}
...
const editMeal = async (value: IDish) => {
  const success = await premiumDashboardStore.editDish(value)
  if (success) {
    await getDishes()
  } else {
    console.error('Failed to edit meal:', premiumDashboardStore.error)
  }
}
...
const addMeal = async (value: IDish) => {
  const success = await premiumDashboardStore.addDish(value)
  if (success) {
    await getDishes()
  } else {
    console.error('Failed to add meal:', premiumDashboardStore.error)
  }
}

const getDishes = async () => {
  const { success } = await premiumDashboardStore.fetchDishes()
  if (success) {
    dishes.value = premiumDashboardStore.dishes
  }
}
