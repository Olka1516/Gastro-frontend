<template>
  <div class="sticky top-20 z-40 pt-2 pb-8 sm:top-15 sm:pt-6">
    <nav
      ...
      :style="panelStyle"
      :aria-label="t('showcase.categoryTabsLabel')"
    >
      <div
        ref="trackRef"
      ...
      >
        <button
          ...
          type="button"
          class="shrink-0 cursor-pointer rounded-lg border px-4 py-2.5 text-sm font-semibold tracking-tight text-white transition-all duration-200 sm:px-5 sm:py-2.5"
          :class="
            isActive(tab.id)
              ? 'border-transparent'
              : 'border-[#2a2930]/70 bg-[#0f0f11]/50 hover:border-[#3f3e47] hover:bg-[#141318]'
          "
          :style="isActive(tab.id) ? activeTabStyle : undefined"
          :aria-current="isActive(tab.id) ? 'true' : undefined"
...
const displayTabs = computed(() => {
  const lang = props.languageCode?.trim()
  return props.categories.map((category) => ({
    id: category.id,
    key: lang ? `${category.id}:${lang}` : category.id,
    name: lang ? getCategoryDisplayName(category, lang) : (category.name?.trim() ?? ''),
  }))
})
